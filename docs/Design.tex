...
\end{figure}
Der Controller ruft beim TankServer, welcher vom UAClientWrapper erbt, die read-Methode für den entsprechenden Parameter auf
...
Dieser Wert wird zurück an den Controller gegeben. Der UAClientWrapper übernimmt somit die
...
\end{figure}
Bei der Erstellung eines TankClient werden HostName und Port des zugehörigen Servers vom mit \emph{getHostName(tank:TankSelector)} und
\emph{getPort(tank:TankSelector)} vom ClientSettingsWrapper abgerufen. ClientSettingsWrapper ist im MonitoringController zu finden. Es
wird eine RemoteMachine erstellt und mit HostName und Port konfiguriert. Danach erstellt man einen TankClient, der die RemoteMachine
übergeben bekommt und sich mit dem Server verbinden kann.
