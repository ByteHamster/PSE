...
\section{Einleitung}
In der heutigen Zeit dringt die Vernetzung in immer weitere Bereiche ein. So auch in die Industrie und ihre Fertigungsprozesse.
...
Dabei soll das auf Transmission Control Protocol (TCP)/Internet Protocol (IP) basierende Kommunikationsprotokoll OPC Unified Architecture (OPC UA) helfen, Maschienen miteinander interagieren zu lassen.
...
die jeweils eine graphische Benuteroberfläche (GUI) besitzen und auf getrennten virtuelle Maschinen laufen. 
...
wo die Farben durch einen Motor gemischt werden. Daten und Werte werden ebenfalls angezeigt. Dazu gibt es die Möglichkeit die Produkation zu konfiguerien, wie zum Beispiel Zu- und Abflussmengen oder Grenzen für Füllstände. Die visuelle Darstellung passt sich an die Konfiguration an.
Die Überwachungskonsole liest die Daten der Fertigungssimulation und stellt sie optisch ansprechend in einem Cockpit dar
und kann Alarme anzeigen, falls Grenzwerte überschritten werden. Das Cockpit ist ebenfalls konfigurierbar. Es ist etwa möglich Filter für Variablen anzuwenden. Allerdings hat die Überwachungskonsole keinen Einfluss auf die Fertigungssimulation.
