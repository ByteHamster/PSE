...
\begin{itemize}
  ...
  \item \gls{Fertigungssimulation} und \gls{Uberwachungskonsole} k\"onnen auf getrennten Rechnern laufen.
  \item \gls{Jitter} in den Zuflussmengen der \gls{Fertigungssimulation} sorgt f\"ur Variation in den gemessenen Werten.
  \item In der \gls{Uberwachungskonsole} sind Alarme vorgegeben, die der Benutzer an- und ausschalten kann.
  \item In der \gls{Fertigungssimulation} kann der Benutzer die simulierte Fertigung steuern.
\end{itemize}

\subsection{Kannkriterien}
\begin{itemize}
  \item Die \gls{Fertigungssimulation} besitzt vorgegebene \glspl{Makro} zur Bef\"ullung und Entleerung der Tanks, um f\"ur Dynamik zu sorgen.
  \item Es ist m\"oglich, in der \gls{Fertigungssimulation} eigene \glspl{Makro} aufzunehmen und wieder abzuspielen.
  \item Die in der \gls{Uberwachungskonsole} eingestellten Alarme k\"onnen gespeichert und beim n\"achsten Programmstart automatisch
    \"ubernommen werden.
  \item In der \gls{Fertigungssimulation} stehen mehrere verschiedene Fertigungen als Auswahl zur Verf\"ugung.
