...
Die hier vorgestellte Simulation "`OSIP"' (OPC UA Simulator for Industrial Plants) ermöglicht es, die Vorteile der Vernetzung von Maschinen mit \gls{OPC UA} interaktiv
...
\begin{figure}[H]
	\centering
	\includegraphics[scale=0.5]{./media/system-sketch.png}
...
Im Folgenden werden Kriterien aufgelistet, die OSIP umsetzen kann.
Im Verlauf des Entwurfs wird entschieden, welche der Kriterien mit angemessenem Aufwand integriert werden können.
\subsubsection{Allgemein}
\begin{itemize}
...
Im Folgenden wird erläutert, was OSIP explizit nicht leisten soll.
\subsubsection{Allgemein}
\begin{itemize}
...
Im Folgenden wird dargestellt, wozu OSIP verwendet werden soll.
...
Feste Anforderungen an die Funktionalität von OSIP werden im Folgenden aufgelistet. Die Nummern optionaler Funktionalitäten, die sich aus den Kann-Kriterien ergeben, sind \textcolor{blue}{blau und mit Sternchen (*)} markiert.
...
Im Folgenden werden Randbedingungen zur Qualität von OSIP spezifiziert.
...
Im Folgenden wird spezifiziert, welche der von OSIP erhobenen Daten persistent gespeichert werden.
\begin{enumerate}
...
Im Folgenden werden Testfälle definiert, die bei einer Auslieferung der Software durchführbar sein müssen.
...
Im Folgenden werden typische Interaktionen mit dem Programm beschrieben.
Dazu werden zuerst die Anwendungsfalldiagramme gezeigt und anschließend die Anwendungsfälle im Detail erläutert.
...
Im Folgenden werden vorläufige Entwürfe der \gls{GUI} dargestellt und erklärt.
\subsection{Fertigungssimulation}
\begin{figure}[H]
  \centering
