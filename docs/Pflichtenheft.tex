...
Der Doppelpfeil verdeutlicht die Beziehung zwischen \glspl{OPC UA Server} (S*) und
dem \gls{OPC UA Client} (C), die den Datenaustausch unterhalb der \gls{Fertigungssimulation} und
...
\begin{itemize}
  ...
  \item Die eingestellten Alarme k\"onnen gespeichert und beim n\"achsten Programmstart automatisch
    \"ubernommen werden.
  \item Die Standard-Sprache Englisch kann über \glspl{Java Property Datei} auf andere Sprachen umgestellt werden.
\end{itemize}
...
Die \gls{Fertigungssimulation} schickt \glspl{Sensordatum} an einen \gls{OPC UA Server} und zeigt
...
Die Daten werden dann in der \gls{GUI} der \gls{Uberwachungskonsole} angezeigt.
\begin{center}
