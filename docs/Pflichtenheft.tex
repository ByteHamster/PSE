...
In der Systemskizze ist links die Fertigungssimulation bzw. stellvertretend ihre GUI skizziert.
Darüber lässt sich der simulierte Mischprozess steuern. Rechts davon wird die Überwachungskonsole dargestellt.
Sie zeigt die Daten der Fertigungssimulation an.
Der Doppelpfeil verdeutlicht die Beziehung zwischen den OPC UA Servern (S*) und
dem OPC UA Client (C), die den Datenaustausch unterhalb der Fertigungssimulation und der Überwachungskonsole regeln.
...
Die Fertigungssimulation schickt Sensordaten an OPC UA Server und zeigt gleichzeitig den simulierten Prozess graphisch an. Diese Werte werden von dem OPC UA Client angefordert und von betreffenden OPC UA Server an den OPC UA Client übermittelt. Unabhängig vom OPC UA Client schicken OPC UA Server bei Bedarf Alarme an den OPC UA Client. Die Überwachungskonsole fordert die übermittelten Daten bei dem OPC UA Client an und bekommt diese von ihm zurückgegeben.
Alarme werden der Überwachungskonsole ohne Anfordern übermittelt.
Die Daten werden dann in der GUI der Überwachungskonsole angezeigt.
\begin{center}
